...
async function fetchTailors(inName, inCity) {
	try {
		let query = supabase.from('tailor').select('*, user(id, name, city)');
...
export async function fetchAllTailors() {
	try {
		let { data, error } = await supabase.from('tailor').select('*');
		if (error) {
			throw error;
		}
		console.log(data);
		...
		console.error('Error fetching all tailors:', error.message);
		return [];
	}
}
